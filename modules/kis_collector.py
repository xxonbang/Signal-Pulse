...
class KISDataCollector:
    ...
    def collect_top50_full(
        ...
        start_time = datetime.now(KST)
        print("\n" + "=" * 60)
        print(f"[KIS] 거래대금 Top50 종목 수집 시작: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        # 3. 고유 종목 코드 합산
        unique_codes = list({s["code"] for s in kospi_top50 + kosdaq_top50})
